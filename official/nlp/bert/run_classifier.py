...
def run_bert(strategy,
  ...
  if FLAGS.mode == 'transfer_learning_and_predict':
    ...
    with strategy.scope():
                  ...
                  if 'transformer_encoder' in bert_sub_layer.name:
                      ...
                      for k, transformer_sub_layer in enumerate(transformer_encoder_layer.layers):
                          if 'word_embeddings' in transformer_sub_layer.name:
                              logging.info(f'word_embeddings eetting weights [{i}], [{j}]: {pretrain_model.layers[i].layers[j].layers[k].get_weights()[10]}')
      ...
      for i, layer in enumerate(classifier_model.layers):
          if 'transformer_encoder' in layer.name:
              ...
              for j, transformer_sub_layer in enumerate(transformer_encoder_layer.layers):
                  if 'word_embeddings' in transformer_sub_layer.name:
                      logging.info(f'word_embeddings setting weights [{i}], [{j}]: {classifier_model.layers[i].layers[j].get_weights()[10]}')
      ...
      logging.info(f'word_embeddings_weights Transfer: {word_embeddings_weights[100]}')
      logging.info(f'word_embeddings_weights Task    : {word_embeddings_weights_class[100]}')
